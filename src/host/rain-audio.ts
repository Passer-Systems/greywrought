/** A seamless stereo wash with a brighter scatter of individual nearby drops. */
export function createRainAudio(context: BaseAudioContext, output: AudioNode) {
  ...
  for (let channel = 0; channel < 2; channel++) {
    ...
    const decay = Math.exp(-1 / (context.sampleRate * .004));
    for (let i = 0; i < frames; i++) {
      const white = random() * 2 - 1;
      low = low * .985 + white * .015;
      mid = mid * .75 + white * .25;
      if (random() < 28 / context.sampleRate) splash = .3 + random() * .5;
      splash *= decay;
      data[i] = low * .9 + mid * .55 + white * (.12 + splash);
    }
  ...
  filter.type = 'lowpass'; filter.frequency.value = 6500; filter.Q.value = .5;
  gain.gain.value = 0;
  ...
  return {
    update(intensity: number, shelter: 'outdoors' | 'cave' | 'underwater', audible: boolean) {
      ...
      const level = audible ? intensity * .72 * attenuation : 0;
      const cutoff = shelter === 'underwater' ? 320 : shelter === 'cave' ? 950 : 6500;

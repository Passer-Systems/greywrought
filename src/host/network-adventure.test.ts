...
test('route submission awaits its actual receipt and forwards intermediate preview stops', async () => {
  ...
  const submitted = game.submitBait(destination, via, 2).then(value => { accepted = value; });
  const message = socket.sent.at(-1)!;
  expect(message).toMatchObject({ type: 'command', command: { type: 'bait', destination, via, waitTicks: 2 } });
  ...
  const next = game.submitBait(destination, via, 2), receipt = socket.sent.at(-1)!;
  ...
  const preview = game.previewBait(destination, via, 2);
  const request = socket.sent.at(-1)!;
  expect(request).toMatchObject({ type: 'command', command: { type: 'previewBait', destination, via, waitTicks: 2 } });
  ...
  const pending = game.submitBait(destination, via, 2); socket.close();
  expect(await pending).toBe(false);
  expect(await game.submitBait(destination, via, 2)).toBe(false);
  game.close();
});

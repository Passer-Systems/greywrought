...
export function createNpcQuests(
  ...
  function render(): void {
    ...
    if (view.status !== "completed" || completedHere === view.id) {
      ...
      const facts = [earned.gear ? `${gearName(earned.gear, snapshot.player.archetype)} (${GEAR[earned.gear].damageReduction ? "2 armor" : "+3 attack damage"})` : "", earned.potions ? `${earned.potions} potions` : "", earned.supplies ? `${earned.supplies} supplies` : "", earned.level > 1 ? `Level ${earned.level}` : ""].filter(Boolean);

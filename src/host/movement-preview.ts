...
export function createMovementPreview(request: (destination: Position, via: readonly Position[]) => Promise<CombatForecast | null>) {
  let key = '', generation = 0, forecast: CombatForecast | null = null, pending = false;
  return {
    get forecast() { return forecast; },
    get pending() { return pending; },
    update(snapshot: AdventureSnapshot, destination: Position | null, via: readonly Position[] = []) {
      const next = destination && snapshot.combat.phase === 'preparation' && !snapshot.combat.ready ? movementPreviewKey(snapshot, destination, via) : '';
      if (next === key) return;
      key = next; const revision = ++generation; forecast = null; pending = Boolean(next);
      if (!next || !destination) return;
      void request(destination, via).then(result => {

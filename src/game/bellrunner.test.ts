...
import { BELLRUNNER_STOPS, bellrunnerDock, flightDuration, flightPosition } from './bellrunner.js';
...
test('every Bellrunner route lifts, crosses the world and lands on walkable town ground', () => {
  for (const from of BELLRUNNER_STOPS) for (const to of BELLRUNNER_STOPS) if (from !== to) {
    const save=JSON.parse(createAdventure().save()); save.state.position=bellrunnerDock(from.id); save.state.coins=71; save.state.supplies=29;
    ...
    expect(restored.snapshot.player.position).toEqual(bellrunnerDock(to.id));
...
test('shared spectators see flight and reconnect/save restoration retains the route', () => {
  ...
  world.join('watcher','Watcher','mage'); travel(rider,-8,-28,world);
  ...
  world.advance(40);
  expect(rider.snapshot.player.position).toEqual(bellrunnerDock('brinewick'));
...
test('local prediction follows the flying rider instead of applying gravity or walking', () => {
  const game=createAdventure(); travel(game,-8,-28); game.fly('suture'); game.advance(6);
  ...
  expect(prediction.player.position).toEqual(bellrunnerDock('suture'));
});

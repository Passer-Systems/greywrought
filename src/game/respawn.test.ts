...
describe("two-minute world regrowth", () => {
  ...
  test("restart preserves the remaining wait, including downtime, without advancing combat or characters offline", () => {
    ...
    expect(JSON.parse(restored.save()).world.threats[0]).toEqual(paused.world.threats[0]);
    ...
    expect(midway.instances).toEqual(paused.instances);
    expect(midway.world.threats[0]).toMatchObject({ phase: "patrol", health: 96, aggro: false, targetPlayerId: null, combatants: [], actionSequence: 0 });
    expect(midway.world.threats[0].position).not.toEqual(paused.world.threats[0].position);
  ...
  test("each harvested batch returns independently after 120 seconds and retains its deadline through restart", () => {
    ...
    const world = createSharedAdventure({ save: JSON.stringify(saved), now: () => time });
    const player = world.join("a", "Ada", "mage");
    tap(player, "gather"); world.advance(2); expect(player.snapshot.resourceRemaining).toBe(9);
    time += 2000; world.advance(2);
    tap(player, "gather"); world.advance(2); expect(player.snapshot.resourceRemaining).toBe(6);
    world.leave("a");

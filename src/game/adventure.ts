...
import { classAction, classKit, MELEE_RANGE, SPRINT_COST } from "./class-kit.js";

type Vector = { x: number; y: number; z: number };
type Phase = AdventureSnapshot["phase"];
export const COMBAT_RULES = {
  actionCooldown: 1,
  bait: { duration: COMBAT_TURN.moveDuration, cost: 0 },
  swarm: { radius: 3, damage: 36 },
  classes: { counterattackDamage: 12, focusDamage: 10, exposureDamage: 6, repositionDamage: 8, residueDamage: 12, residueRadius: 3 },
  window: { active: COMBAT_TURN.duration, choosing: 0, preparation: 30, gathering: 5 },
  stamina: { maximum: 100, recoveryPerTurn: 20 },
  strike: { damage: 18, range: MELEE_RANGE, rangedRange: 10, stopDistance: 1.5, duration: 0.25, cost: 0 },
  brace: { block: 24, duration: 2, cost: 0 },
  ...
  fireRush: { damage: 16, overshoot: 3, duration: .65, radius: .85, burnDamage: 4, burnInterval: .75, burnDuration: 6 },
  wolf: { circleRange: 5.5, circleRadius: 4.5, circleSpeed: 1.5, lungeDistance: 8, lungeHeight: 0.9, impactRadius: 2 },
} as const;
export const PLAYER_COMBAT_ACTIONS = ["strike", "brace", "bait", "special"] as const;
...
interface Maneuver {
  kind: "lunge" | "bait"; targetId: string; remainingSeconds: number; duration: number;
...
type HeadAbilityId = "fire-rush" | "ember-beam" | "fireball" | "ember-ward" | "kindle";
interface FireTrail {
  id: number; points: Vector[]; remainingSeconds: number; combatants: string[]; cooldowns: Record<string, number>;
}
interface HeadState {
  rush: { start: Vector; destination: Vector; hitPlayers: string[] } | null;
  ...
  fireballs: { id: number; origin: Vector; position: Vector; direction: Vector | null; targetPlayerId: string | null; remainingSeconds: number; duration: number; damage: number }[];
}
...
interface ThreatState {
  slowedCycle: number;
  volatileResidue: boolean; exposed: boolean;
...
interface CombatState { sprinting: boolean; clock: CombatClock; queued: QueueEntry[]; nextId: number; ready: boolean; }
const newClock = (): CombatClock => ({ gatheringRemainingSeconds: 0, pendingSeconds: 0, phase: "idle", elapsedSeconds: 0, cycle: 0 });
const newCombat = (clock = newClock()): CombatState => ({ clock, sprinting: false, queued: [], nextId: 1, ready: false });
...
interface State {
  ...
  focusReady: boolean; counterattackReady: boolean; repositioned: boolean;
  energyCombatActive: boolean;
...
type SavedState = Omit<State, "world" | "combat"> & WorldState & { combat: CombatClock & Pick<CombatState, "queued" | "nextId" | "ready" | "sprinting"> };
function savedState(state: State): SavedState {
  const { world, combat, ...player } = state;
  return { ...player, ...world, combat: { ...combat.clock, queued: combat.queued, nextId: combat.nextId, ready: combat.ready, sprinting: combat.sprinting } };
}
...
const DEFINITIONS: readonly ThreatDefinition[] = [
  ...
  { id: "patrol", level: 2, behavior: "wolf", disposition: "hostile", aggroRange: 6, leash: 30, speed: 4.2, name: "Ash hound", position: point(-17, 45), health: 72,
    patrol: [point(-17, 45), point(-22, 39), point(-26, 43), point(-24, 51), point(-18, 53)],
  ...
  { id: "meadow-rat-2", critter: true, level: 1, disposition: "neutral", aggroRange: 0, leash: 14, speed: 1.2, pursuitSpeed: 2,
    ...
    patrol: [point(-15, 17), point(-19, 23), point(-14, 27), point(-8, 22), point(-11, 15)],
  ...
  { id: "meadow-rat-3", critter: true, callsForHelp: false, level: 1, disposition: "neutral", aggroRange: 0, leash: 14, speed: 1.15, pursuitSpeed: 1.8,
    name: "Fern rat", position: point(-24, 27), health: 24,
    patrol: [point(-24, 27), point(-28, 31), point(-25, 35), point(-20, 31)],
    preparation: "Ruffling its whiskers", intention: "Little bite", damage: 2, reach: 1.2,
    benefit: "A small forager nosing through the western ferns." },
  { id: "meadow-bird-4", critter: true, callsForHelp: false, level: 1, disposition: "neutral", aggroRange: 0, leash: 16, speed: 1.3, pursuitSpeed: 2,
    name: "Emberfinch", position: point(15, 14), health: 18,
    patrol: [point(15, 14), point(21, 16), point(24, 21), point(19, 19), point(14, 17)],
    preparation: "Shaking its copper feathers", intention: "Flutter away", damage: 2, reach: 1.2,
    benefit: "A bright little finch circling the quieter eastern grass." },
...
const newHead = (): HeadState => ({ rush: null, fireTrails: [], opened: false, ability: "ember-beam", castVolley: 1, block: 0, blockSeconds: 0, volley: 1, projectileSequence: 0, pendingFireballs: 0, nextFireballSeconds: 0, fireballs: [] });
const newThreat = (t: ThreatDefinition): ThreatState => ({
  ...
  slowedCycle: 0, volatileResidue: false, exposed: false, comboOpened: false, staggered: false, swarm: null, contributors: [], combatants: [], rollClaims: [], shield: 0,
...
function initialState(archetype: CharacterArchetype): State {
  return {
    ...
    focusReady: false, counterattackReady: false, repositioned: false, guardSeconds: 0, block: 0, energyCombatActive: false, stamina: 100, staminaRecoverySeconds: 0, maneuver: null, sitting: false,
...
class Adventure implements AdventureGame {
  ...
  private effectId = 0;
  private recording: { actions: CombatForecast["actions"][number][]; paths: CombatForecast["paths"][number][]; events: CombatForecast["events"][number][]; outcomes: CombatForecast["outcomes"][number][] } | null = null;
  ...
  static sharedAdventure(options: Pick<AdventureOptions, "save" | "now">): SharedAdventure {
    ...
    if (options.save !== undefined) {
      ...
      if ((root.version !== 1 && root.version !== 2 && root.version !== 3 && root.version !== 4 && root.version !== 5 && root.version !== 6) || root.kind !== "shared-adventure" || !Array.isArray(root.characters)) throw new Error("Unsupported shared adventure save.");
      const world = record(root.world), version = root.version === 1 ? 9 : root.version >= 6 ? 12 : root.version >= 4 ? 11 : 10;
      const template = savedState(initialState("warrior"));
      if (version < 12) template.stamina = 5;
    ...
    const beginViewing = (ctx: SharedContext): void => {
      ...
      if (ctx.clock.phase === 'active') ctx.online.values().next().value?.recoverTurnEnergy();
      ...
      for (const [id, game] of ctx.characters) {
        ...
        game.state.energyCombatActive = false;
    ...
    return {
      ...
      advance(seconds) {
        ...
        for (const ctx of new Set(sessions.values())) {
          ...
          if (ctx.mode === 'private' && ctx.combatOccurred && !ctx.world.threats.some(t => t.head?.fireballs.length) && ![...ctx.characters.values()].some(game => game.state.health > 0 && game.inCombat())) beginViewing(ctx);
          if (ctx.mode === 'viewing' && ctx.returnStartedAt !== null && (ctx.now() - ctx.returnStartedAt >= 15_000 || allConfirmed(ctx))) materialize(ctx);
        }
      },
      save() {
        refresh();
        return JSON.stringify({ version: 6, spatialLayout: 1, terrainLayout: TERRAIN_LAYOUT, forestLayout: 1, kind: "shared-adventure", world: context.world, clock: context.clock,
  ...
  private playerView(): AdventureSnapshot["player"] {
    ...
    return {
      ...
      focusReady: s.focusReady, counterattackReady: s.counterattackReady, repositioned: s.repositioned,
      movementTiles: this.movementTiles(), movementRange: this.movementTiles() * COMBAT_CELL_SIZE, combatMovementSpeed: this.combatMovementSpeed(),
  ...
  get snapshot(): AdventureSnapshot {
    ...
    return {
      ...
      phase: s.phase, combat: { sprinting: s.combat.sprinting, gatheringRemainingSeconds: s.combat.clock.gatheringRemainingSeconds, openingStrikeAvailable: this.openingStrikeAvailable(), forecast: this.forecast(), hazards: s.world.threats.flatMap(t => [...(t.head?.fireTrails.flatMap(trail => trail.points.map((position, index) => ({ id: t.id + ":fire:" + trail.id + ":" + index, kind: "fire" as const, position: { ...position }, radius: COMBAT_RULES.fireRush.radius, remainingSeconds: trail.remainingSeconds }))) ?? []), ...(t.swarm ? [{ id: t.id + ":swarm", kind: "swarm" as const, position: { ...t.swarm.position }, radius: COMBAT_RULES.swarm.radius }] : []), ...(t.volatileResidue ? [{ id: t.id + ":residue", kind: "residue" as const, position: { ...t.position }, radius: COMBAT_RULES.classes.residueRadius }] : [])]), effects: this.effects.map(e => ({ ...e, position: { ...e.position } })), ready: s.combat.ready, phase: s.combat.clock.phase, remainingSeconds: s.combat.clock.phase === "idle" ? 0 : Math.max(0, (s.combat.clock.phase === "active" ? COMBAT_RULES.window.active : COMBAT_RULES.window.preparation) - s.combat.clock.elapsedSeconds), elapsedSeconds: s.combat.clock.elapsedSeconds, cycle: s.combat.clock.cycle, queued: s.combat.queued.map(e => ({ ...e })), reservedStamina: this.reservedStamina(), availableStamina: s.stamina - this.reservedStamina() },
  ...
  save(): string { return JSON.stringify({ version: 12, spatialLayout: 1, terrainLayout: TERRAIN_LAYOUT, forestLayout: 1, state: savedState(this.state) }); }
  ...
  private actionUnlocked(action: CombatMove["action"]): boolean {
    return action === "strike" || action === "brace" || action === "bait" || action === "special";
  }
  ...
  private beginExecution(): void {
    ...
    for (const player of this.combatants()) {
      ...
      if (move?.destination) {
        const route = validateCombatRoute(player.state.position, [...move.via, move.destination], player.movementTiles(), [...player.occupiedCells(player.state.position), ...destinations]);
        if (!route) {
    ...
    for (const threat of this.state.world.threats) if (threat.phase === "preparation" && !threat.wolf) {
      threat.approaching = threat.head?.ability !== "fire-rush" && this.ability(threat).profile.aim !== "self" && distance(threat.position, this.pursuitTarget(threat)) > this.ability(threat).range + EPSILON;
    }
  ...
  private beginPlanning(): void {
    const clock = this.state.combat.clock;
    if (clock.phase === "active") this.recoverTurnEnergy();
    ...
    for (const player of this.participants()) {
      player.state.combat.queued = []; player.state.combat.ready = false; player.state.combat.sprinting = false;
      player.state.guardSeconds = 0; player.state.block = 0; player.state.repositioned = false;
      if (!player.inCombat()) player.state.energyCombatActive = false;
    ...
    if (!trio[0]!.comboOpened && trio.every(t => t.aggro && t.health > 0 && t.joinCycle <= clock.cycle)) {
      for (const threat of trio) {
        threat.comboOpened = true; threat.specialOffset = threat.wolf ? .85 : threat.head ? .6 : 1.7;
  ...
  private finishCombatStep(dt: number): void {
    const clock = this.state.combat.clock;
    for (const player of this.participants()) if (player.inCombat()) player.startCombatEnergy();
    const engaged = this.combatants().length > 0;
    if (!engaged && !this.state.world.threats.some(t => t.head?.fireballs.length)) {
      if (this.recording) this.captureOutcomes();
      if (clock.phase === "active") this.recoverTurnEnergy();
      for (const player of this.participants()) player.state.energyCombatActive = false;
      for (const threat of this.state.world.threats) if (threat.head) { threat.head.fireTrails = []; threat.head.rush = null; }
      clock.phase = "idle"; clock.elapsedSeconds = 0; clock.gatheringRemainingSeconds = 0;
      for (const player of this.participants()) { player.state.combat.queued = []; player.state.combat.ready = false; player.state.combat.sprinting = false; }
      return;
    }
  ...
  private movementTiles(): number { return classKit(this.state.archetype).movementTiles * (this.state.combat.sprinting ? 2 : 1); }
  private combatMovementSpeed(): number { return classKit(this.state.archetype).combatMovementSpeed * (this.state.combat.sprinting ? 2 : 1); }
  setSprint(active: boolean): boolean {
    const s = this.state, c = s.combat;
    if (!this.editableQueue() || !this.inCombat() || c.clock.phase !== "preparation") return false;
    const movement = c.queued.find(entry => entry.action === "bait" && entry.status === "pending");
    if (active && s.stamina - this.reservedStamina() + (movement?.cost ?? 0) < SPRINT_COST) {
      this.report("Sprint needs 30 energy.", "combat"); return false;
    }
    if (!active && movement?.destination && !validateCombatRoute(s.position, [...movement.via, movement.destination], classKit(s.archetype).movementTiles, this.occupiedCells(s.position))) {
      this.report("Shorten or clear your route before turning Sprint off.", "combat"); return false;
    }
    c.sprinting = active;
    if (movement) movement.cost = active ? SPRINT_COST : 0;
  ...
  private queueReason(action: CombatAction, excludedId?: number): string | null {
    ...
    if (s.stamina - pending.reduce((sum, entry) => sum + entry.cost, 0) < this.actionCost(action)) return "Not enough energy.";
    if (action === "special" && !this.inCombat()) return "Skills are available during combat.";
    if (action === "strike" || action === "special" && classAction(s.archetype, action).target === "unit") {
      const target = s.world.threats.find(threat => threat.id === s.selectedThreat);
      if (!target?.active || target.health <= 0 || target.phase === "returning" || (action === "special" || this.inPrivateInstance()) && !target.aggro) return "Select a living enemy in this encounter.";
  ...
  private derivePlanTiming(): void {
    const queued = this.state.combat.queued, movement = queued.find(e => e.action === "bait"), hasMovement = !!movement;
    const duration = movement?.destination ? combatRouteDistance(this.state.position, [...movement.via, movement.destination]) / this.combatMovementSpeed() : 0;
    for (const entry of queued) entry.offsetSeconds = entry.action === "bait" ? COMBAT_TURN.moveStart + (movement?.waitTicks ?? 0) * COMBAT_TURN.waitTick : actionTimingOffset(entry.timing!, hasMovement, duration, movement?.waitTicks);
  }
  queueBait(destination: Position, via: readonly Position[] = [], waitTicks = 0): boolean {
    ...
    if (!Number.isInteger(waitTicks) || waitTicks < 0 || waitTicks > COMBAT_TURN.maxWaitTicks) return false;
    ...
    const route = validateCombatRoute(this.state.position, [...via, destination], this.movementTiles(), this.occupiedCells(this.state.position));
    ...
    if (existing) { existing.destination = target; existing.via = stops; existing.waitTicks = waitTicks; }
    else c.queued.push({ id: c.nextId++, action: "bait", destination: target, via: stops, waitTicks, targetId: null, timing: null, offsetSeconds: COMBAT_TURN.moveStart, cost: this.actionCost("bait"), status: "pending", reason: null });
    this.derivePlanTiming(); c.ready = false; return true;
  }
  private queueAction(action: CombatAction): void {
    ...
    const target = action === "strike" || action === "special" && classAction(this.state.archetype, action).target === "unit" ? this.state.world.threats.find(t => t.id === this.state.selectedThreat)! : null;
    if (action === "strike" && target && this.openingStrikeAvailable()) {
    ...
    if (target) {
      this.state.sitting = false; this.activeEmote = null;
      if (target.aggro && !this.inCombat()) { this.startCombatEnergy(); this.settleCombatCell(); }
    ...
    const timing = existing?.timing ?? "after";
  ...
  setActionTiming(timing: CombatActionTiming): boolean {
    ...
    if (!this.editableQueue() || c.clock.phase !== "preparation" || !entry || !["before", "during", "after"].includes(timing)) return false;
  ...
  private actionCost(action:CombatMove["action"]):number{return action === "special" ? classAction(this.state.archetype, "special").cost! : action === "bait" && this.state.combat.sprinting ? SPRINT_COST : 0;}
  private useAbility(action:"brace", committed = false):void{const s=this.state,cost=this.actionCost(action);if(!committed&&!this.ready()){this.report("You are still recovering.","combat");return;}if(s.stamina<cost){this.report("Not enough energy.","combat");return;}this.spendStamina(cost);this.recover(action,COMBAT_RULES.actionCooldown);s.guardSeconds=committed?COMBAT_TURN.duration:COMBAT_RULES.brace.duration;s.block=classAction(s.archetype,"brace").block??COMBAT_RULES.brace.block;if(s.archetype==="mage"){s.focusReady=true;this.classFeedback("Focus ready (+10; Move cancels)");}const h=this.inCombat()?Math.min(classAction(s.archetype,"brace").heal??0,100-s.health):0;s.health+=h;this.feedback(null,"heal",h);this.report("You gain "+s.block+" block "+(committed?"for the rest of this turn.":"for "+s.guardSeconds+" seconds.")+(h?" Restored "+h+" health.":""),"combat");}
  ...
  setAction(action: AdventureAction, pressed: boolean): void {
    ...
    if (["forward", "backward", "left", "right", "jump", "dive", "strike", "special", "bait", "brace", "drinkPotion", "ritual", "cancelGather"].includes(action)) this.cancelGather();
    this.act(action);
  }
  ...
  private act(action: AdventureAction): void {
    ...
    switch (action) {
      ...
      case "strike":
      case "special":
        if (s.phase === "expedition") this.queueAction(action);
        break;
  ...
  private ability(t: ThreatState): ThreatAbilityView {
    if (t.head) return headAbility(t.head.ability, t.head.castVolley, t.id === "glassmire-lantern");
    if (t.wolf) return maulAbility(t.damage);
    if (t.id === "ritual-guardian") return foremanAbility(t.abilityIndex, t.damage);
    if (t.id === "warder") return wardenAbility(t.abilityIndex, t.damage);
  ...
  private phaseDuration(t: ThreatState): number {
    ...
    if (t.phase === "action") return t.head?.ability === "fire-rush" ? COMBAT_RULES.fireRush.duration : t.head ? COMBAT_RULES.head.fireballTravel + (t.head.castVolley - 1) * COMBAT_RULES.head.fireballSpacing : t.wolf ? COMBAT_RULES.enemy.action : this.ability(t).noticeSeconds;
    return 0;
  }
  ...
  private attackFailure(t: ThreatState, action: "strike" | "special" = "strike"): "target-unavailable" | "out-of-range" | "behind-cover" | null {
    const s = this.state;
    const range = classAction(s.archetype, action).range ?? COMBAT_RULES.strike.range;
    if (s.phase !== "expedition" || !t.active || t.health <= 0 || t.phase === "returning" || this.inPrivateInstance() && !t.aggro) return "target-unavailable";
    if (distance(s.position, t.position) > range + EPSILON) return "out-of-range";
    if (!this.attackPath(t)) return "behind-cover";
    if (this.inCombat() && t.aggro) return null;
    const settled = this.engagementPositions(t);
    if (distance(settled.player, settled.threat) > range + EPSILON) return "out-of-range";
    return this.clearPath(settled.player, settled.threat) ? null : "behind-cover";
  }
  ...
  private hit(t: ThreatState, damage: number, verb: string): void {
    ...
    const s = this.state, residue = t.volatileResidue, exposed = t.exposed;
    const empowered = s.archetype === "warrior" && s.counterattackReady;
    const repositioned = s.archetype === "hunter" && s.repositioned;
    if (empowered) { damage += COMBAT_RULES.classes.counterattackDamage; s.counterattackReady = false; this.classFeedback("Counterattack +12 damage", t.id); }
    if (repositioned) { damage += COMBAT_RULES.classes.repositionDamage; s.repositioned = false; this.classFeedback("Repositioned shot +8 damage", t.id); }
    if (s.archetype === "mage" && s.focusReady) { damage += COMBAT_RULES.classes.focusDamage; s.focusReady = false; this.classFeedback("Focused Attack +10 damage", t.id); }
    if (exposed) { damage += COMBAT_RULES.classes.exposureDamage; t.exposed = false; this.classFeedback("Exposed weak point +6 damage", t.id); }
    ...
    const dealt = Math.min(damage - blocked, t.health);
    t.health -= dealt;
    ...
    if (residue) this.igniteResidue(t, this.playerId ?? "solo");
    else if (t.health > 0 && s.archetype === "alchemist") {
      t.volatileResidue = true; this.classFeedback("Volatile residue applied", t.id);
    }
    if (t.health > 0 && s.archetype === "artificer" && !exposed) {
      t.exposed = true; this.classFeedback("Weak point exposed (+6)", t.id);
    }
    this.defeat(t, this.playerId ?? "solo");
  }
  private useSpecial(target: ThreatState | undefined): void {
    const s = this.state, spec = classAction(s.archetype, "special"), source = this.playerId ?? "solo";
    const origin = { ...s.position }, center = s.archetype === "alchemist" ? { ...target!.position } : origin;
    const radius = s.archetype === "alchemist" ? 3 : spec.target === "self" ? spec.range! : 0;
    let destination = target ? { ...target.position } : undefined;
    if (s.archetype === "hunter") {
      const direction = this.direction(origin, target!.position);
      destination = { x: origin.x + direction.x * spec.range!, y: origin.y, z: origin.z + direction.z * spec.range! };
      // Match the same solid-cover test used by attacks, and end the arrow at its first obstruction.
      for (let length = .1; length <= spec.range!; length += .1) {
        const point = { x: origin.x + direction.x * length, y: origin.y, z: origin.z + direction.z * length };
        if (!this.clearPath(origin, point)) { destination = { x: origin.x + direction.x * (length - .1), y: origin.y, z: origin.z + direction.z * (length - .1) }; break; }
      }
    }
    const enemies = s.world.threats.filter(t => t.active && t.health > 0 && t.aggro && t.phase !== "returning" &&
      (s.archetype === "hunter" ? this.segmentTouches(origin, destination!, t.position, .65) && distance(origin, t.position) <= spec.range! + EPSILON && this.clearPath(origin, t.position)
        : radius ? distance(center, t.position) <= radius + EPSILON && this.clearPath(center, t.position) : t === target));
    this.spendStamina(spec.cost!); this.recover("special", spec.duration!); s.attackSequence++; s.presence++;
    this.tracePath(source, "attack", "special", radius ? [center] : [origin, destination!], radius);
    const kinds = { warrior: "whirlwind", hunter: "piercing-arrow", mage: "frost-nova", alchemist: "volatile-flask", artificer: "disruptor-shot" } as const;
    for (const player of this.participants()) {
      player.effects.push({ id: ++player.effectId, kind: kinds[s.archetype], position: { ...center }, ...(!radius && destination ? { destination } : {}), radius });
      if (player.effects.length > 16) player.effects.shift();
    }
    const coated = enemies.filter(t => t.volatileResidue);
    for (const enemy of enemies) {
      if (s.archetype === "mage") enemy.slowedCycle = s.combat.clock.cycle;
      if (s.archetype === "artificer" && this.hasUnresolvedCast(enemy)) this.interrupt(enemy, source);
      this.enemyHit(enemy, spec.damage!, source);
    }
    if (s.archetype === "alchemist") {
      for (const enemy of coated) {
        // Defeat clears coatings, but a flask still ignites the coating it struck.
        enemy.volatileResidue = true; this.igniteResidue(enemy, source);
      }
      for (const enemy of enemies) if (enemy.health > 0 && !coated.includes(enemy)) enemy.volatileResidue = true;
    }
    this.report(`${spec.name} hits ${enemies.length} ${enemies.length === 1 ? "enemy" : "enemies"}.`, "combat");
  }
  private classFeedback(text: string, targetId: string | null = null): void {
    this.report(text + ".", "combat");
    this.traceEvent("class", this.playerId ?? "solo", targetId, this.state.position, 0, text);
  }
  private igniteResidue(t: ThreatState, sourceId: string): void {
    if (!t.volatileResidue) return;
    t.volatileResidue = false;
    const position = { ...t.position }, radius = COMBAT_RULES.classes.residueRadius;
    this.traceEvent("ignition", sourceId, t.id, position, COMBAT_RULES.classes.residueDamage, "Volatile residue ignites!", radius);
    for (const player of this.participants()) {
      player.effects.push({ id: ++player.effectId, kind: "ignition", position: { ...position }, radius });
      if (player.effects.length > 16) player.effects.shift();
      player.report("Volatile residue ignites! 12 damage within 3 metres.", "combat");
    }
    for (const enemy of this.state.world.threats) if (enemy.active && enemy.health > 0 && distance(enemy.position, position) <= radius) this.enemyHit(enemy, COMBAT_RULES.classes.residueDamage, sourceId);
    for (const player of this.participants()) if (player.state.health > 0 && distance(player.state.position, position) <= radius) player.hurt(COMBAT_RULES.classes.residueDamage, "Burning residue", false, sourceId);
  }
  private defeat(t: ThreatState, sourceId: string): void {
    if (t.health === 0 && t.phase !== "cleared") {
      ...
      t.shield = 0; t.volatileResidue = false; t.exposed = false;
  ...
  private traceAction(entry: QueueEntry, result: CombatForecast["actions"][number]["result"]): void {
    if (!this.recording) return;
    const actorId = this.playerId ?? "solo";
    const index = this.recording.actions.findIndex(action => action.actorId === actorId && action.queueId === entry.id);
    if (index >= 0) this.recording.actions[index] = { actorId, queueId: entry.id, action: entry.action, targetId: entry.targetId, result };
  }
  ...
  async previewBait(destination: Position, via: readonly Position[] = [], waitTicks = 0): Promise<CombatForecast | null> {
    return this.forecast(destination, via, waitTicks);
  }
  private forecast(destination?: Position, via: readonly Position[] = [], waitTicks = 0): CombatForecast | null {
    ...
    if (destination && !copies[players.indexOf(this)]!.queueBait(destination, via, waitTicks)) return null;
  ...
  private firstCollision(actor: ThreatState, from: Position, to: Position, radius = 1.1): ThreatState | undefined {
    return this.state.world.threats.filter(t => t !== actor && t.active && t.health > 0 && (!this.inPrivateInstance() || t.aggro) && t.phase !== "returning" && this.segmentTouches(from, to, t.position, radius))
  ...
  private interrupt(t: ThreatState, sourceId: string): void {
    ...
    if (t.head) { t.head.pendingFireballs = 0; t.head.rush = null; }
  ...
  private moveManeuver(dt: number): void {
    ...
    if (!m) return;
    const motion = { ...m }, previous = { ...s.position }, previousDistance = m.traveledDistance;
    ...
    if (m.kind === "bait" && s.archetype === "hunter" && previousDistance < COMBAT_CELL_SIZE - EPSILON && m.traveledDistance >= COMBAT_CELL_SIZE - EPSILON) {
  ...
  private returnToTown(): void {
    const s = this.state;
    s.counterattackReady = false; s.repositioned = false; s.focusReady = false;
  ...
  private stepShared(dt: number): void {
    ...
    for (const t of this.state.world.threats) {
      ...
      if (t.aggro && target && t.targetPlayerId !== target.playerId) {
        ...
        if (t.head?.ability !== "fire-rush" && !(t.id === "warder" && t.abilityIndex === 2)) t.targetPosition = { ...target.state.position };
      }
    ...
    if (this.state.combat.clock.phase === "active") {
      ...
      for (const t of this.state.world.threats) {
        ...
        if (t.head?.fireballs.length && (!t.aggro || t.health <= 0)) (target ?? this).advanceFireballs(t, dt);
        else if (t.aggro && t.active && t.health > 0 && target) target.advanceThreat(t, dt);
      }
    }
    this.advanceGroundFire(dt, previousPositions);
  ...
  private executeQueued(): void {
    const s = this.state, c = s.combat;
    for (const entry of [...c.queued].sort((a,b) => Number(b.action === "bait") - Number(a.action === "bait"))) {
      ...
      let result: CombatForecast["actions"][number]["result"] = "executed";
      if (entry.action === "strike" || entry.action === "special") {
        const spec = classAction(s.archetype, entry.action);
        const failure = s.stamina < entry.cost ? "insufficient-stamina" : spec.target === "self" ? null : target && (entry.action !== "special" || target.aggro) ? this.attackFailure(target, entry.action) : "target-unavailable";
        if (entry.timing === "during" && failure && c.clock.elapsedSeconds < COMBAT_TURN.duration - EPSILON) {
          s.selectedThreat = selected; this.executingQueueId = null; continue;
        }
        if (failure === null) {
          if (spec.movementProfile === "stationary" && s.maneuver) {
            const traveled = s.maneuver.traveledDistance;
            s.maneuver = null; this.moving = false;
            const path = this.recording?.paths.slice().reverse().find(p => p.actorId === (this.playerId ?? "solo") && p.kind === "move");
            if (path) {
              const points = [path.points[0]!]; let traversed = 0;
              for (let i = 1; i < path.points.length; i++) {
                const next = path.points[i]!, length = distance(path.points[i-1]!, next);
                if (traversed + length >= traveled - EPSILON) break;
                points.push(next); traversed += length;
              }
              points.push({ ...s.position });
              this.recording!.paths[this.recording!.paths.indexOf(path)] = { ...path, points };
            }
          }
          if (entry.action === "special") this.useSpecial(target);
          else {
            this.tracePath(this.playerId ?? "solo", "attack", "strike", [s.position, target!.position], 0);
            this.recover("strike", COMBAT_RULES.strike.duration);
            this.hit(target!, spec.damage ?? COMBAT_RULES.strike.damage, "Attack at");
          }
        } else {
          result = failure!;
          this.report(failure === "insufficient-stamina" ? "Not enough energy." : failure === "out-of-range" ? "The target is out of reach." : failure === "behind-cover" ? "The target is behind cover." : "The target is no longer available.", "combat");
        }
        entry.status = s.attackSequence > before ? "executed" : "failed";
      } else {
        s.currentAction = null;
        if (entry.action === "bait") {
          // Ready validates actor occupancy; later pursuit cannot revoke an accepted route.
          const route = validateCombatRoute(s.position, [...entry.via, entry.destination!], this.movementTiles());
          ...
          const duration = combatRouteDistance(s.position, route) / this.combatMovementSpeed();
          if (s.stamina < entry.cost) { entry.status = "failed"; entry.reason = "Not enough energy."; this.report(entry.reason, "combat"); this.traceAction(entry, "insufficient-stamina"); s.selectedThreat = selected; this.executingQueueId = null; continue; }
          this.spendStamina(entry.cost); this.recover("bait", duration);
          if (s.focusReady) { s.focusReady = false; this.classFeedback("Move breaks focus"); }
          s.maneuver = { kind: "bait", targetId: s.selectedThreat, start: { ...s.position }, destination: end, via: route.slice(0, -1), traveledDistance: 0, facing, remainingSeconds: duration, duration };
  ...
  private engage(t: ThreatState): void {
    ...
    this.state.sitting = false; this.activeEmote = null;
    const settled = this.engagementPositions(t);
    if (!this.inCombat()) { this.startCombatEnergy(); this.settleCombatCell(); }
  ...
  private beginCast(t: ThreatState): void {
    ...
    if (t.id === "warder") {
      t.abilityIndex = t.actionSequence % 3;
      t.damage = Math.ceil([24, 18, 32][t.abilityIndex]! * (1 + ramp));
      t.specialOffset = [1, .65, 1.2][t.abilityIndex]!;
    }
    if (t.head) {
      t.head.rush = null;
      t.head.ability = this.chooseHeadAbility(t); t.head.castVolley = t.head.volley;
      if (t.head.ability === "fireball") t.specialOffset = .6;
      if (t.head.ability === "fire-rush") t.specialOffset = 1.1;
      t.damage = this.ability(t).damage;
    }
    t.turnTarget = { ...(this.targetPlayer(t)?.state.position ?? this.state.position) };
    t.castDuration = t.specialOffset;
    t.remainingSeconds = t.specialOffset;
    t.targetPosition = t.wolf ? this.wolfEndpoint(t) : { ...t.position };
    if (t.head?.ability === "fire-rush") {
      const direction = this.direction(t.position, t.turnTarget);
      const home = definition(t.id), dx = t.position.x - home.position.x, dz = t.position.z - home.position.z;
      const projection = dx * direction.x + dz * direction.z;
      const limit = Math.max(0, -projection + Math.sqrt(Math.max(0, projection * projection + home.leash ** 2 - dx * dx - dz * dz)) - .01);
      const travel = Math.min(distance(t.position, t.turnTarget) + COMBAT_RULES.fireRush.overshoot, limit);
      t.targetPosition = this.reachableEndpoint(t.position, point(t.position.x + direction.x * travel, t.position.z + direction.z * travel));
    }
    if (t.id === "warder" && t.abilityIndex === 2) t.targetPosition = { ...t.turnTarget };
  ...
  private chooseHeadAbility(t: ThreatState): HeadAbilityId {
    ...
    if (t.id === "scout" && t.actionSequence % 4 === 1) return "fire-rush";
  ...
  private startCombatEnergy(): void {
    if (this.state.energyCombatActive) return;
    this.state.energyCombatActive = true;
    this.state.stamina = COMBAT_RULES.stamina.maximum;
    this.state.staminaRecoverySeconds = 0;
  }
  private recoverTurnEnergy(): void {
    for (const player of this.participants()) if (player.state.energyCombatActive) {
      player.state.stamina = Math.min(COMBAT_RULES.stamina.maximum, player.state.stamina + COMBAT_RULES.stamina.recoveryPerTurn);
    }
  }
  private spendStamina(cost: number): void { this.state.stamina -= cost; }
  private releaseThreat(t: ThreatState): void {
    ...
    t.comboOpened = false; t.slowedCycle = 0; t.staggered = false; t.swarm = null; t.volatileResidue = false; t.exposed = false; t.cancelledWindow = false;
    ...
    if (t.head) { const { fireballs, projectileSequence } = t.head; t.head = { ...newHead(), fireballs, projectileSequence }; }
    const d = definition(t.id);
  ...
  private pursue(t: ThreatState, dt: number): void {
    ...
    const speed = this.pursuitSpeed(t);
  ...
  private pursuitSpeed(t: ThreatState): number {
    const d = definition(t.id);
  ...
  private positionThreat(t: ThreatState, dt: number): void {
    if (this.ability(t).profile.aim === "self") return;
    const target = this.pursuitTarget(t), reach = t.wolf ? COMBAT_RULES.wolf.lungeDistance : this.ability(t).range;
    const gap = distance(t.position, target) - reach;
    const speed = this.pursuitSpeed(t);
    if (gap > EPSILON && speed > 0) this.moveThreat(t, target, Math.min(dt, gap / speed));
    if (!(t.id === "warder" && t.abilityIndex === 2)) t.targetPosition = t.wolf ? this.wolfEndpoint(t) : { ...t.position };
  }
  private advanceThreat(t: ThreatState, dt: number): void {
    ...
    if (t.phase === "preparation") {
      if (t.wolf || t.approaching) this.positionThreat(t, dt);
      if (t.approaching && distance(t.position, this.pursuitTarget(t)) <= this.ability(t).range + EPSILON) t.approaching = false;
      ...
      const reach = t.wolf ? COMBAT_RULES.wolf.lungeDistance : this.ability(t).range;
      if (t.approaching || t.wolf && distance(t.position, this.pursuitTarget(t)) > reach + EPSILON) {
        if (clock.elapsedSeconds >= COMBAT_TURN.duration - EPSILON) this.beginRecovery(t);
        return;
      }
      ...
      } else { t.phase = "action"; t.approaching = false; t.remainingSeconds = this.ability(t).noticeSeconds; if (!(t.id === "warder" && t.abilityIndex === 2)) t.targetPosition = { ...t.position };
    ...
    if (t.phase === "action") {
      ...
      if (t.head) {
        if (t.head.ability === "fire-rush") this.advanceFireRush(t, dt);
        else this.advanceFireballs(t, dt);
        return;
      }
  ...
  private resolveHeadCast(t: ThreatState): void {
    ...
    if (h.ability === "fire-rush") {
      h.rush = { start: { ...t.position }, destination: { ...t.targetPosition }, hitPlayers: [] };
      h.fireTrails.push({ id: t.actionSequence, points: [{ ...t.position }], remainingSeconds: COMBAT_RULES.fireRush.burnDuration, combatants: [...t.combatants, t.targetPlayerId ?? "solo"], cooldowns: {} });
      t.phase = "action"; t.remainingSeconds = COMBAT_RULES.fireRush.duration;
      this.tracePath(t.id, "attack", "fire-rush", [t.position, t.targetPosition], COMBAT_RULES.fireRush.radius);
      this.report("Cinder Watchman rushes along its marked path, leaving burning ground.", "combat");
      return;
    }
  ...
  private advanceFireRush(t: ThreatState, dt: number): void {
    const h = t.head!, rush = h.rush;
    if (!rush) { this.beginRecovery(t); return; }
    const old = { ...t.position };
    t.remainingSeconds = Math.max(0, t.remainingSeconds - dt);
    const fraction = 1 - t.remainingSeconds / COMBAT_RULES.fireRush.duration;
    t.position = point(rush.start.x + (rush.destination.x - rush.start.x) * fraction, rush.start.z + (rush.destination.z - rush.start.z) * fraction);
    t.moving = distance(old, t.position) > EPSILON;
    const trail = h.fireTrails.find(trail => trail.id === t.actionSequence)!;
    if (distance(trail.points.at(-1)!, t.position) >= .35 || t.remainingSeconds <= EPSILON) trail.points.push({ ...t.position });
    const traced = this.recording?.paths.find(path => path.actorId === t.id && path.action === "burning-ground:" + trail.id);
    if (traced) (traced.points as Position[]).splice(0, traced.points.length, ...trail.points.map(p => ({ ...p })));
    for (const player of this.participants()) {
      const id = player.playerId ?? "solo";
      if (!trail.combatants.includes(id) || rush.hitPlayers.includes(id) || player.state.health <= 0 || player.state.flight) continue;
      if (this.segmentTouches(old, t.position, player.state.position, COMBAT_RULES.fireRush.radius) && Math.abs(player.state.position.y - t.position.y) < 2) {
        rush.hitPlayers.push(id); t.lastActionHit = true;
        player.hurt(t.damage, "Cinder Watchman — Fire Rush", false, t.id);
      }
    }
    if (t.remainingSeconds <= EPSILON) { h.rush = null; this.beginRecovery(t); }
  }
  private advanceGroundFire(dt: number, previousPositions: ReadonlyMap<string, Vector>): void {
    if (this.state.combat.clock.phase !== "active") return;
    for (const threat of this.state.world.threats) for (const trail of threat.head?.fireTrails ?? []) {
      if (trail.remainingSeconds <= EPSILON) continue;
      if (this.recording && !this.recording.paths.some(path => path.actorId === threat.id && path.action === "burning-ground:" + trail.id)) {
        this.tracePath(threat.id, "attack", "burning-ground:" + trail.id, trail.points, COMBAT_RULES.fireRush.radius);
      }
      // Overlapping patches share a pulse; crossing uses the actual movement segment.
      for (const player of this.participants()) {
        const id = player.playerId ?? "solo";
        trail.cooldowns[id] = Math.max(0, (trail.cooldowns[id] ?? 0) - dt);
        if (!trail.combatants.includes(id) || player.state.health <= 0 || player.state.phase !== "expedition" || player.state.flight || trail.cooldowns[id] > EPSILON) continue;
        const from = previousPositions.get(id) ?? player.state.position;
        if (!trail.points.some(p => Math.abs(player.state.position.y - p.y) < 1.5 && this.segmentTouches(from, player.state.position, p, COMBAT_RULES.fireRush.radius))) continue;
        trail.cooldowns[id] = COMBAT_RULES.fireRush.burnInterval;
        player.hurt(COMBAT_RULES.fireRush.burnDamage, "Burning ground", false, threat.id);
      }
      trail.remainingSeconds = Math.max(0, trail.remainingSeconds - dt);
    }
    for (const threat of this.state.world.threats) if (threat.head) threat.head.fireTrails = threat.head.fireTrails.filter(trail => trail.remainingSeconds > EPSILON);
  }
  private advanceFireballs(t: ThreatState, dt: number): void {
    const h = t.head!, profile = headAbility("fireball", h.castVolley, t.id === "glassmire-lantern").profile;
    for (const ball of [...h.fireballs]) {
      const old = { ...ball.position };
      const target = this.participants().find(player => player.playerId === ball.targetPlayerId) ?? this;
      if (!ball.direction || profile.aim === "tracking") ball.direction = this.direction(old, target.state.position);
      const travel = definition(t.id).reach / COMBAT_RULES.head.fireballTravel * Math.min(dt, ball.remainingSeconds);
      const intended = point(old.x + ball.direction.x * travel, old.z + ball.direction.z * travel);
      const end = this.reachableEndpoint(old, intended), wall = distance(end, intended) > EPSILON;
      const enemy = profile.friendlyFire ? this.firstCollision(t, old, end, .75) : undefined;
      const players = this.participants().filter(player => player.state.health > 0 && player.state.phase === "expedition" && !player.state.flight && this.segmentTouches(old, end, player.state.position, .75));
      players.sort((a,b) => distance(old, this.contactPoint(old, end, a.state.position, .75)) - distance(old, this.contactPoint(old, end, b.state.position, .75)));
      const player = players[0];
      const enemyContact = enemy ? this.contactPoint(old, end, enemy.position, .75) : null;
      const playerContact = player ? this.contactPoint(old, end, player.state.position, .75) : null;
      const hitsEnemy = enemyContact && (!playerContact || distance(old, enemyContact) < distance(old, playerContact));
      ball.position = hitsEnemy ? enemyContact! : playerContact ?? end;
      ...
      for (const bee of this.state.world.threats) if (bee.swarm && (!this.inPrivateInstance() || bee.aggro) && this.segmentTouches(old, ball.position, bee.swarm.position, COMBAT_RULES.swarm.radius)) this.ignite(bee, t.id);
      for (const coated of this.state.world.threats) if (coated.volatileResidue && (!this.inPrivateInstance() || coated.aggro) && this.segmentTouches(old, ball.position, coated.position, .75)) this.igniteResidue(coated, t.id);
      if (hitsEnemy) { this.enemyHit(enemy!, ball.damage, t.id); ball.remainingSeconds = 0; }
      else if (player) { player.hurt(ball.damage, definition(t.id).name + " — Fireball", false, t.id); ball.remainingSeconds = 0; }
      else if (wall) ball.remainingSeconds = 0;
    }
    ...
    if (t.health > 0 && !t.staggered && h.pendingFireballs > 0 && h.nextFireballSeconds <= EPSILON) {
      if (this.headInRange(t)) {
        const ball = { id: ++h.projectileSequence, origin: point(t.position.x, t.position.z), position: point(t.position.x, t.position.z), direction: this.direction(t.position, this.state.position), targetPlayerId: this.playerId, remainingSeconds: COMBAT_RULES.head.fireballTravel, duration: COMBAT_RULES.head.fireballTravel, damage: COMBAT_RULES.head.fireballDamage };
  ...
  private resolveAttack(t: ThreatState): void {
    ...
    if (!t.lastActionHit) { this.feedback(null, "miss", 0); this.report(`${definition(t.id).name} — ${this.intention(t)} misses you.`, "combat"); }
  }
  ...
  private hurt(damage: number, source: string, bypassBlock = false, sourceId = source): void {
    ...
    s.block -= blocked;
    if (blocked > 0 && s.archetype === "warrior" && !s.counterattackReady) {
      s.counterattackReady = true; this.classFeedback("Counterattack ready (+12)");
    }
    ...
    this.report(`${source} hits you for ${taken} damage${blocked > 0 ? ` (${blocked} blocked by Defend)` : ""}.`, "combat");
    if (s.health > 0) return;
    this.cancelGather();
    s.maneuver = null; s.block = 0; s.guardSeconds = 0; s.counterattackReady = false; s.repositioned = false; s.focusReady = false;
...
function readSave(serialized: string, now = Date.now()): State {
  ...
  migrateSpatialLayout(root);
  migrateTerrainLayout(root);
  const version = number(root.version, 1, 12, true), realtime = version >= 10;
  ...
  const threats: ThreatState[] = s.threats.map(value => {
    ...
    const savedMaximum = t.maximumHealth === undefined ? (id === "cave-crab" ? 156 : id === "warder" ? 72 : d.health) : number(t.maximumHealth, 1);
    const savedHealth = number(t.health, 0, Math.max(savedMaximum, d.health));
    const health = id === "ritual-guardian" && t.active === false && s.chapter === undefined ? d.health : Math.min(d.health, savedHealth > 0 && id === "warder" ? savedHealth + Math.max(0, d.health - savedMaximum) : savedHealth);
    ...
    const result: ThreatState = {
      ...
      slowedCycle: t.slowedCycle === undefined ? 0 : number(t.slowedCycle, 0, Number.MAX_SAFE_INTEGER, true),
      staggered: t.staggered === undefined ? false : boolean(t.staggered),
      volatileResidue: t.volatileResidue === undefined ? false : boolean(t.volatileResidue),
      exposed: t.exposed === undefined ? false : boolean(t.exposed),
  ...
  const state: State = {
    ...
    currentAction: realtime && typeof s.currentAction === "string" && ["hearthstone", "bait", "strike", "brace", "special", "drinkPotion", "gather", "ritual", "equip"].includes(s.currentAction) ? choice(s.currentAction, ["hearthstone", "bait", "strike", "brace", "special", "drinkPotion", "gather", "ritual", "equip"] as const) : null,
    ...
    focusReady: s.focusReady === undefined ? false : boolean(s.focusReady),
    counterattackReady: s.counterattackReady === undefined ? false : boolean(s.counterattackReady),
    repositioned: s.repositioned === undefined ? false : boolean(s.repositioned),
    guardSeconds: number(s.guardSeconds, 0, 5),
    block: version >= 4 ? number(s.block, 0, 28) : number(s.guardSeconds, 0, 3) > 0 ? 5 : 0,
    energyCombatActive: version >= 12 ? boolean(s.energyCombatActive) : threats.some(t => t.aggro && t.health > 0),
    stamina: version >= 12 ? number(s.stamina, 0, 100, true) : version >= 8 ? number(s.stamina, 0, 5, true) * 20 : 100,
  ...
  const sprinting = version >= 12 && rawCombat?.sprinting !== undefined ? boolean(rawCombat.sprinting) : false;
  ...
  for (const e of savedQueue) {
    if (e.action !== "bait" && e.action !== "strike" && e.action !== "brace" && e.action !== "special") continue;
    ...
    queued.push({ id: number(e.id,1,Number.MAX_SAFE_INTEGER,true), action, targetId: e.targetId === null ? null : text(e.targetId), destination: e.destination === undefined || e.destination === null ? null : groundPosition(e.destination), via: action === "bait" ? readRouteStops(e.via) : [], ...(action === "bait" ? { waitTicks: e.waitTicks === undefined ? 0 : number(e.waitTicks,0,COMBAT_TURN.maxWaitTicks,true) } : {}), timing, offsetSeconds: number(e.offsetSeconds,0,COMBAT_TURN.duration), cost: action === "bait" && sprinting ? SPRINT_COST : classAction(player.archetype, action).cost ?? 0, status: choice(e.status,["pending","executed","failed"] as const), reason: e.reason === null ? null : text(e.reason) });
  }
  ...
  const movement = queued.find(e => e.action === "bait");
  const duration = player.maneuver?.duration ?? (movement?.destination ? combatRouteDistance(player.position, [...movement.via, movement.destination]) / (classKit(player.archetype).combatMovementSpeed * (sprinting ? 2 : 1)) : 0);
  if (clock.phase !== "active") for (const entry of queued) entry.offsetSeconds = entry.action === "bait" ? COMBAT_TURN.moveStart + (movement?.waitTicks ?? 0) * COMBAT_TURN.waitTick : actionTimingOffset(entry.timing!, !!movement, duration, movement?.waitTicks);
  return { ...player, combat: { clock, queued, sprinting, ready: legacyPlan || rawCombat?.ready === undefined ? false : boolean(rawCombat.ready), nextId: rawCombat ? number(rawCombat.nextId, 1, Number.MAX_SAFE_INTEGER, true) : 1 }, world: { threats: restoredThreats, resourceRemaining, resourceRespawns, ritualCalled, chestClaimed } };
}
...
function readManeuver(value: unknown): Maneuver | null {
  ...
  const duration = m.duration === undefined ? (kind === "bait" ? COMBAT_RULES.bait.duration : COMBAT_RULES.strike.duration) : number(m.duration, Number.EPSILON, COMBAT_TURN.moveDuration);
...
function readDirection(value: unknown): Vector { const d = record(value); return { x: number(d.x, -1, 1), y: 0, z: number(d.z, -1, 1) }; }
...
function readHead(value: unknown, realtime: boolean): HeadState {
  ...
  const rush = h.rush === undefined || h.rush === null ? null : record(h.rush);
  const fireTrails = h.fireTrails === undefined ? [] : h.fireTrails;
  if (!Array.isArray(fireTrails)) throw new Error("Invalid adventure save: invalid burning ground.");
  return { rush: rush ? { start: groundPosition(rush.start), destination: groundPosition(rush.destination), hitPlayers: stringList(rush.hitPlayers) } : null,
    fireTrails: fireTrails.map(value => { const f = record(value); return { id: number(f.id, 0, Number.MAX_SAFE_INTEGER, true), points: readRouteStops(f.points), remainingSeconds: number(f.remainingSeconds, 0, COMBAT_RULES.fireRush.burnDuration), combatants: stringList(f.combatants), cooldowns: Object.fromEntries(Object.entries(record(f.cooldowns)).map(([id, value]) => [id, number(value, 0, COMBAT_RULES.fireRush.burnInterval)])) }; }),
    opened: boolean(h.opened), ability: choice(h.ability, ["fire-rush", "ember-beam", "fireball", "ember-ward", "kindle"] as const), volley, castVolley,
    ...
    fireballs: h.fireballs.map(value => { const p = record(value); return { id: number(p.id, 1, Number.MAX_SAFE_INTEGER, true), origin: groundPosition(p.origin), position: p.position === undefined ? groundPosition(p.origin) : groundPosition(p.position), direction: p.direction === undefined || p.direction === null ? null : readDirection(p.direction), targetPlayerId: p.targetPlayerId === undefined || p.targetPlayerId === null ? null : text(p.targetPlayerId), remainingSeconds: number(p.remainingSeconds, 0, COMBAT_RULES.head.fireballTravel), duration: number(p.duration, COMBAT_RULES.head.fireballTravel, COMBAT_RULES.head.fireballTravel), damage: number(p.damage, COMBAT_RULES.head.fireballDamage, COMBAT_RULES.head.fireballDamage) }; }),
  };
}

function foremanAbility(index: number, damage: number): ThreatAbilityView {
  if (index === 2) return { profile: { aim: "self", movement: "hold", friendlyFire: false }, id: "foreman-shield", name: "Safety Shield", description: "Raises 60 Block for the rest of the turn. Attack before the shield rises.", damage: 0, range: 0, noticeSeconds: 0 };
  if (index === 0) return { profile: { aim: "self", movement: "hold", friendlyFire: false }, id: "foreman-pulse", name: "Roll-call Pulse", description: "Pulses within 22 metres. Cannot be dodged or stopped by cover. Defend before the pulse.", damage, range: 22, noticeSeconds: .35 };
  return { profile: { aim: "ground", movement: "hold", friendlyFire: true }, id: "foreman-press", name: "Final Press", description: "Strikes the marked ground. Move away or Defend before impact.", damage, range: 3.5, noticeSeconds: .35 };
}
function wardenAbility(index: number, damage: number): ThreatAbilityView {
  if (index === 1) return { profile: { aim: "tracking", movement: "hold", friendlyFire: false }, id: "warden-cut", name: "Relic Cut", description: "A quick cut at its opponent within 3 metres, 0.2 seconds after windup. Defend or retreat out of reach; sidestepping within reach does not evade it.", damage, range: 3, noticeSeconds: .2 };
  if (index === 2) return { profile: { aim: "ground", movement: "hold", friendlyFire: true }, id: "warden-smash", name: "Sealbreaker", description: "Commits to your starting ground, then smashes the marked 2.5-metre area after a long windup. Move out of the mark or Defend. The impact can hit other enemies.", damage, range: 2.5, noticeSeconds: .65 };
  return { profile: { aim: "ground", movement: "hold", friendlyFire: true }, id: "warden-sweep", name: "Relic Sweep", description: "Holds its ground and sweeps within 5 metres, 0.7 seconds after winding up. Retreat clear or Defend. Nearby enemies can be caught in the sweep.", damage, range: 5, noticeSeconds: .7 };
}
function headAbility(id: HeadAbilityId, volley: number, homing = false): ThreatAbilityView {
  if (id === "fire-rush") return { profile: { aim: "direction", movement: "lunge", friendlyFire: false }, id, name: "Fire Rush", description: "Aims through its target to 3 metres beyond them before you plan, stopping at cover or the edge of its territory. Rushes after 1.1 seconds for 16 damage. Step sideways to bait it past you. The trail burns for 6 seconds of combat, dealing 4 damage on contact and every 0.75 seconds; planning pauses the flames. Defend absorbs damage.", damage: COMBAT_RULES.fireRush.damage, range: definition("scout").leash * 2, noticeSeconds: COMBAT_RULES.fireRush.duration };
  if (id === "ember-beam") return { profile: { aim: "tracking", movement: "hold", friendlyFire: false }, id, name: "Ember Beam", description: "Deals 8 damage during the turn within 10 metres. Defend before impact, or plan a move behind cover or out of reach before it fires.", damage: COMBAT_RULES.head.beamDamage, range: 10, noticeSeconds: 0 };
  if (id === "ember-ward") return { profile: { aim: "self", movement: "hold", friendlyFire: false }, id, name: "Ember Ward", description: "Raises 6 Block for 2 seconds during the turn. Attack before the shield rises or recover while it holds.", damage: 0, range: 10, noticeSeconds: 0 };
  if (id === "kindle") return { profile: { aim: "self", movement: "hold", friendlyFire: false }, id, name: "Kindle", description: "Adds one fireball to every later volley during the turn. Use the opening to attack.", damage: 0, range: 0, noticeSeconds: 0 };
  return { profile: { aim: homing ? "tracking" : "direction", movement: "hold", friendlyFire: true }, id, name: "Fireball ×" + volley, description: "Launches " + volley + (homing ? " seeking fireballs that follow you" : " straight fireballs that lock direction when fired") + " for 18 damage each. Shots cover up to 10 metres in 0.9 seconds; launches are 0.2 seconds apart. " + (homing ? "Defend or put cover or another enemy in their path; stepping aside alone will not avoid them." : "Move aside after launch, Defend, or put cover or another enemy in their path.") + " Fired shots survive interruption. Fire ignites spilled swarms for 36 damage within 3 metres, including you.", damage: COMBAT_RULES.head.fireballDamage * volley, range: 10, noticeSeconds: .9 };
}
function maulAbility(damage = 18): ThreatAbilityView {
  return { profile: { aim: "ground", movement: "lunge", friendlyFire: true }, id: "maul", name: "Lunging Maul", description: "Leaps up to 8 metres during the turn, landing 0.65 seconds later in a 2-metre area. Bait its leap through another enemy: the collision damages and staggers both, interrupting their attacks. Each Maul gains 2 damage, up to 36.", damage, range: COMBAT_RULES.wolf.impactRadius, noticeSeconds: .65 };
}
...
export function getMonsterLore(): readonly MonsterLoreEntry[] {
  return DEFINITIONS.map(d => {
    ...
    if (d.behavior === "head") return {
      ...
      abilities: [...(d.id === "scout" ? [headAbility("fire-rush",1)] : []), headAbility("ember-beam",1), headAbility("fireball",1,d.id === "glassmire-lantern"), headAbility("ember-ward",1), headAbility("kindle",1)],
      sequences: [
        ...(d.id === "scout" ? [{ name: "Fire Rush", abilityIds: ["fire-rush"], offsetsSeconds: [], description: "After the opening Beam and every fourth action thereafter, commits to a straight rush that leaves burning ground. Plan a sidestep before it launches." }] : []),
        ...
        { name: "Ember Ward", abilityIds: ["ember-ward"], offsetsSeconds: [], description: "Shields when below half health, if you have energy and are close enough to strike. Never shields twice in a row." },
    ...
    if (d.behavior === "wolf") return {
      id: d.id, name: d.name, health: d.health, disposition: d.disposition,
      description: d.description ?? "Patrols the western trail. Approaches until within 8 metres, then commits to a leap. Nearby hostile allies answer its call.",
    ...
    return { id:d.id, name:d.name, health:d.health, disposition:d.disposition,
      ...
      abilities: [...(d.id === "warder" ? [wardenAbility(0,24),wardenAbility(1,18),wardenAbility(2,32)] : [ordinaryAbility(d)]), ...(d.id === "warder" ? [{ profile: { aim: "tracking" as const, movement: "hold" as const, friendlyFire: false }, id: "harvest-thorns", name: "Live cabling", description: "Gathering while the Relic Warden lives deals 8 damage. Block absorbs it.", damage: 8, range: 0, noticeSeconds: 0 }] : [])],
      sequences: [{ name: d.id === "warder" ? "Sweep, Cut, Sealbreaker" : d.intention, abilityIds:d.id === "warder" ? ["warden-sweep","warden-cut","warden-smash"] : [d.id], offsetsSeconds:[], description:"Commits one attack per turn, then chooses again before the next plan." }],

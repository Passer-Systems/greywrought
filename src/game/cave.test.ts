...
test('old five-creature solo and shared saves retain progress and acquire cave creatures',()=>{
  const currentIds = getMonsterLore().map(threat => threat.id).sort();
  expect(currentIds).toEqual(expect.arrayContaining(['cave-bat', 'cave-crab']));
  ...
  expect(game.snapshot.threats.map(threat => threat.id).sort()).toEqual(currentIds);
  ...
  expect(player.snapshot.threats.map(threat => threat.id).sort()).toEqual(currentIds);expect(restored.pause('caver')).toBe(true);
  const fork=createSharedAdventure({save:restored.save()});
  expect(fork.join('caver','Caver','warrior').snapshot.threats.map(threat => threat.id).sort()).toEqual(currentIds);

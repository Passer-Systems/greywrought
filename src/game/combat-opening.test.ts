...
test('an unaware enemy takes one immediate opening hit with no duplicate queued attack', () => {
  ...
  game.advance(4.99); expect(game.snapshot.combat.phase).toBe('preparation');
...
test('detected attacks queue normally and Ready cannot skip the five-second opening', () => {
  ...
  expect(game.snapshot.threats[0]!.health).toBe(before);
  game.advance(.01);
...
test('social helpers join opening planning without rerolling casts or clearing the plan and Ready', () => {
  ...
  expect(game.snapshot.combat.gatheringRemainingSeconds).toBeCloseTo(4.9, 7);
});
...
test('party pause and restored saves freeze the collection clock while forecasts still execute a full turn', () => {
  ...
  restored.advance(remaining); expect(returned.snapshot.combat.phase).toBe('active');

...
| 2.5 s target window | Actual ms | rAF intervals ms median/p95/max (n) | Projection→HUD ms median/p95/max (n) | Candidate wall ms median/p95/max (n) | Admission wall ms median/p95/max (n) | Received candidate requests/s |
| --- | ---: | --- | --- | --- | --- | ---: |
...
Observed rAF callback rates were 0.38, 1.17, and 0.39 per wall second. With only
1/3/1 interval samples these windows demonstrate long SwiftShader stalls but do
not support a stable FPS distribution. The observed intervals missed the labeled
16.67 ms aspiration by roughly two orders of magnitude; this host does not
establish hardware-GPU frame behavior. The fixed tick is 16 ms (62.5 requested
ticks per second), while the main thread received candidate-requested receipts
at 30.23–55.30 per wall second. That is not a count of completed language Steps
or Admissions: scheduling backlog can move receipt delivery across window
boundaries. Main-thread projection/HUD work
...
reached 564.8/1730.2/1835.4 ms. Candidate and Admission durations pair worker
timestamps and measure elapsed wall time under SwiftShader and the shared quota;
they are not isolated CPU-time attribution. Their tails—not the ordinary HUD
projection—are the measured semantic steady-state cost to inspect.
...
limits, and the normal-input action recipe under
`build/measurement/m5-baseline.json`.

That accepted ignored JSON is schema v1 and retains the legacy keys
`simulatedSteps` / `simulatedStepsPerWallSecond` and `replaySlice`. Read those
keys as received candidate-request receipts and a replay action recipe,
respectively. The corrected tracked driver emits schema v2 names
`receivedCandidateRequests`, `receivedCandidateRequestsPerWallSecond`, and
`replayActionRecipe`. No rerun was needed for this interpretation-only repair.
...
attributing the sparse SwiftShader rAF result to product rendering. The retained
action recipe is not an exact pre-state/input/output comparison fixture. A fair
conventional comparison still needs such an isolated fixture and must never
become a second Greywrought engine. That comparison, actual

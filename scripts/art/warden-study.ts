/** bun scripts/art/warden-study.ts [--export-only]
 * Authors a separate review model; never overwrites the live Warden or source pack.
 */
...
const build='build/warden-study',output='3d/relic-warden-study';
await mkdir(build,{recursive:true});await mkdir(output,{recursive:true});
const sourcePath='3d/Animated Mech Pack - March 2021/Flat Colors/glTF/Mike.gltf';
...
const root=new T.Group();root.name='Relic Warden / surface study';root.add(source.scene);root.scale.setScalar(scale);root.position.y=-box.min.y*scale;
root.userData={sourcePack:'Quaternius Animated Mech Pack — March 2021',sourceCharacter:'Mike',sourceLicense:'CC0-1.0',sourcePath,loreSource:'docs/legacy-lore.json',canonStatus:'New visual design, not a pre-existing named lore character',authoring:'Individually drawn quad control surfaces, resolved with Blender subdivision and solidify. All visible geometry newly authored. Original joint hierarchy and exact animation tracks retained.',studyStatus:'Visual review study; not installed as a live mob',authoredParts:panels.length};
...
await Bun.write(`${output}/relic-warden.glb`,full);
...
await copyFile('3d/Animated Mech Pack - March 2021/License.txt',`${output}/QUATERNIUS-LICENSE.txt`);
...
console.log(`Warden: ${triangles} study triangles; ${runtimeTriangles} game triangles; ${panels.length} surfaces; ${source.animations.length} original clips; full ${(full.byteLength/1048576).toFixed(2)} MiB; game ${(runtime.byteLength/1048576).toFixed(2)} MiB.`);

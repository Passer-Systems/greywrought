...
export async function checkMinimap(page: Awaited<ReturnType<typeof openBrowser>>, location: string): Promise<void> {
  ...
  const sample = await page.evaluate<{ colors: number; centered: boolean; round: boolean; span: number }>(`(() => {
    ...
    return {colors:colors.size,centered:arrow.style.left==='50%'&&arrow.style.top==='50%',round:getComputedStyle(document.getElementById('map-field')).borderRadius==='50%',span:Number(canvas.dataset.span)};
  })()`);
  check(sample.colors > 30, `${location}: minimap contains shaded scenery detail`);
  check(sample.centered && sample.round, `${location}: centered arrow and circular map`);

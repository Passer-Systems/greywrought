...
const saved = JSON.parse(seed.save()), position = { x: 47, y: terrainHeight(47, -46), z: -46 };
...
for (const enemy of saved.world.threats) {
  if (enemy.id === 'cave-bat') Object.assign(enemy, { position: { ...position, x: 49 }, targetPosition: position, aggro: true, targetPlayerId: character.id, combatants: [character.id], phase: 'preparation', joinCycle: 1, windowCycle: 1 });
...
check(fixturePlayer.snapshot.player.inCombat, 'Fixture begins within its cave enemy’s home range');
...
try {
  ...
  page = await openBrowser('camera-obstruction', { beforeNavigate: async call => {
  ...
  await page.waitFor('document.body.dataset.rigState==="ready"&&document.body.dataset.environmentState==="ready"&&window.cameraState?.player.inCombat&&window.camera');
  ...
  await page.waitFor('JSON.parse(document.getElementById("world-canvas").dataset.moveTiles).length>3&&cutaway.cutawayGrid.value.w>2');
  await Bun.sleep(400);
  const pixels = await page.evaluate<{ changed: number; unchanged: number; restored: number; groundPatched: boolean; instanceCount: number }>(`(()=>{
    ...
    const faded=draw(1),solid=draw(0),again=draw(1);let changed=0,unchanged=0,restored=0;
    for(let i=0;i<faded.length;i+=4){const d=Math.abs(faded[i]-solid[i])+Math.abs(faded[i+1]-solid[i+1])+Math.abs(faded[i+2]-solid[i+2]);if(d>6)changed++;else unchanged++;if(Math.abs(faded[i]-again[i])+Math.abs(faded[i+1]-again[i+1])+Math.abs(faded[i+2]-again[i+2])>6)restored++;}
    ...
    return {changed,unchanged,restored,groundPatched,instanceCount};})()`);
  console.log('rendered cutaway', JSON.stringify(pixels));
  check(pixels.changed > 1000, 'Cutaway actually reveals pixels through the cave roof');
  check(pixels.unchanged > pixels.changed * 2, 'Most of the world remains visually intact');

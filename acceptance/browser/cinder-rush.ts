...
try {
  ...
  page = await openBrowser('cinder-rush', { beforeNavigate: async call => {
    ...
    await call('Page.addScriptToEvaluateOnNewDocument', { source: `
      ...
      window.WebSocket=class extends Native { constructor(url,...args) { super(String(url).includes('/world')?'ws://127.0.0.1:4506/world':url,...args); this.addEventListener('message',event=>{const data=JSON.parse(event.data); if(data.type==='state'){window.profileState=data.snapshot;window.profileHistory.push(data.snapshot);}}); } };` });
  } });

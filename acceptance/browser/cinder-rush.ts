...
const url = 'http://127.0.0.1:4505/';
Object.assign(Bun.env, { GREYWROUGHT_GAME_URL: url, GREYWROUGHT_DEBUG_PORT: '9705', GREYWROUGHT_VULKAN: '1' });
const character = { id: 'cinder-rush-fixture', name: 'Bait Tester', archetype: 'warrior' as const, createdAtMillis: 1 };
const token = 'cinder-rush-fixture-token-00000000000000';
...
for (const enemy of saved.world.threats) {
  ...
  if (enemy.id === 'scout') {
    Object.assign(enemy, { health: enemy.maximumHealth, active: true, aggro: true, lootClaimed: false, respawnAt: null,
      ...
      position: enemy.id === 'scout' ? point(-5,30) : point(2.5,31.65), targetPlayerId: character.id,
      combatants: [character.id], joinCycle: 1, windowCycle: 1, specialOffset: 1.1, castDuration: 1.1, remainingSeconds: 1.1, damage:16, actionSequence:1, turnTarget:point(0,30),targetPosition:point(3,30) });
    if (enemy.head) Object.assign(enemy.head, { opened: true, ability: 'fire-rush' });
  }
}
const savePath = `${process.cwd()}/build/browser/cinder-rush-${process.pid}.json`;
...
const server = Bun.serve<WorldSocketData>({ hostname: '127.0.0.1', port: 4506, fetch: (request, host) => service.fetch(request,host), websocket: service.websocket });
const frontend = Bun.spawn([process.execPath, 'scripts/dev-server.ts'], { env: { ...Bun.env, GREYWROUGHT_PORT: '4505' }, stdout: Bun.file('build/browser/cinder-rush-frontend.log'), stderr: Bun.file('build/browser/cinder-rush-errors.log') });
let page: Awaited<ReturnType<typeof openBrowser>> | undefined;
try {
  for (let attempt=0;attempt<100;attempt++) { try { if ((await fetch(url)).ok) break; } catch {} await Bun.sleep(100); }
  page = await openBrowser('cinder-rush', { beforeNavigate: async call => {
    ...
    await call('Page.addScriptToEvaluateOnNewDocument', { source: `
      ...
      window.WebSocket=class extends Native { constructor(url,...args) { super(String(url).includes('/world')?'ws://127.0.0.1:4506/world':url,...args); this.addEventListener('message',event=>{const data=window.decodeWorldMessage(event); if(data.type==='state'){window.profileState=data.snapshot;window.profileHistory.push(data.snapshot);}}); } };` });
  } });
  ...
  check(await page.evaluate(`document.querySelector('${shot}').textContent.includes('Fire Rush')`),'Planner names the committed Fire Rush');
  check(await page.evaluate(`document.querySelector('${shot} .combat-plan-enemy-behavior').textContent==='Committed rush'`),'Planner distinguishes a committed rush from a projectile');
  await page.click(shot);
  await page.shot('committed-rush-warning');
  const eyeImage = await page.evaluate<string>(`(async()=>{
    const {Vector3}=await import('three'),scene=window.profileScene,renderer=window.profileRenderer,rig=scene.children.find(object=>object.userData.threatId==='scout');
    const camera=window.profileCamera.clone(),center=rig.position.clone().add(new Vector3(0,2.25,0));
    const forward=new Vector3(0,0,1).applyQuaternion(rig.quaternion);
    camera.position.copy(center).addScaledVector(forward,4.3).add(new Vector3(0,.25,0));camera.lookAt(center);camera.updateMatrixWorld();
    renderer.render(scene,camera);return renderer.domElement.toDataURL('image/png').split(',')[1];
  })()`);
  await Bun.write(page.output+'/watchman-socket-flames.png',Buffer.from(eyeImage,'base64'));
  ...
  await page.waitFor('document.getElementById("combat-plan-outcome").textContent.includes("No damage")');
  ...
  await page.shot('sidestep-rush-preview');
  ...
  check(forecast.paths.some(path=>path.action==='fire-rush'&&path.points.at(-1)?.x===3),'The committed forecast retains the straight rush endpoint');
  ...
  check(actual.combat.hazards.some(hazard=>hazard.kind==='fire'),'Rush leaves actual burning ground');
  check(await page.evaluate('JSON.parse(document.getElementById("world-canvas").dataset.burningGroundTiles).length>0'),'Burning ground warns occupied combat tiles');
  for (const id of [character.id,'scout']) {
  ...
  await page.shot('burning-ground-after-dodge');
  check(page.errors.length===0,'No browser exceptions');
  console.log('PASS committed rush warning, clickable sidestep, burn tiles and matching networked execution',page.output);

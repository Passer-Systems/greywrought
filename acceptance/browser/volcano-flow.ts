...
const saved=JSON.parse(seed.save());Object.assign(saved.characters[0].state,{phase:'expedition',position:{x:60,y:terrainHeight(60,15),z:15}});
...
const frontend=Bun.spawn([process.execPath,'scripts/static-server.ts'],{env:{...Bun.env,GREYWROUGHT_PORT:'4482',GREYWROUGHT_LOCAL_WORLD:'0'},stdout:Bun.file('build/browser/volcano-frontend.log'),stderr:Bun.file('build/browser/volcano-frontend-errors.log')});
...
try{
 ...
 await page.call('Input.dispatchMouseEvent',{type:'mouseMoved',x:486,y:240,button:'right',buttons:2});

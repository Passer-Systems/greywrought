...
import { createSharedAdventure, getMonsterLore } from "../../src/game/adventure.js";
...
try {
  ...
  for (const [model, asset] of cases) {
    const id = asset.encounterId;
    const lore = getMonsterLore().find(threat => threat.id === id)!;
    const name = lore.name;
    const character = { id: "art-test-" + id, name: "Art Tester", archetype: "mage" as const, createdAtMillis: 1 };
    const token = "art-fixture-token-000000000000000000";
    ...
    const underground = target.position.y < -1;
    const position = { x: target.position.x - (underground ? 8 : 0), y: 0, z: target.position.z - (underground ? 0 : 10) };
    ...
    try {
      ...
      check(await page.evaluate(`artState.threats.find(t=>t.id===${JSON.stringify(id)}).name===${JSON.stringify(name)}`), "Updated creature name must reach the client");
      check(await page.evaluate(`performance.getEntriesByType('resource').some(e=>e.name.endsWith(${JSON.stringify(actorAssetPath(model))}))`), `${name} asset must load`);
      await page.evaluate(`(async()=>{const {Mesh,SkinnedMesh}=await import('three');window.artPoses=[];Mesh.prototype.onBeforeRender=function(){for(let p=this;p;p=p.parent)if(p.userData.threatId===${JSON.stringify(id)}){if(this instanceof SkinnedMesh){const s=Array.from(this.skeleton.boneMatrices).map(n=>n.toFixed(3)).join(',');if(!artPoses.includes(s)&&artPoses.length<40)artPoses.push(s);}break;}};})()`);
      const distance = `(()=>{const t=artState.threats.find(t=>t.id===${JSON.stringify(id)}),p=artState.player.position;return Math.hypot(t.position.x-p.x,t.position.z-p.z)})()`;
      async function approach(range: number, stopOnCombat = false) {
        const held = new Set<string>(), deadline = performance.now() + 25000;
        try {
          while (performance.now() < deadline) {
            const delta = await page!.evaluate<{ x: number; z: number; combat: boolean }>(`(()=>{const t=artState.threats.find(t=>t.id===${JSON.stringify(id)}),p=artState.player.position;return {x:t.position.x-p.x,z:t.position.z-p.z,combat:artState.player.inCombat}})()`);
      ...
      for (let cycle = 0; cycle < 5; cycle++) {
        const health = await page.evaluate<number>(`artState.threats.find(t=>t.id===${JSON.stringify(id)}).health`);
        if (health === 0) break;
        await page.click('.adventure-actions [data-action="strike"]');
        await page.waitFor('artState.combat.phase==="preparation"', 10000);
        const sequence = await page.evaluate<number>("artState.combat.cycle");
        await page.press("KeyR");
        await page.waitFor(`artState.combat.phase!=="active" && (artState.combat.cycle>${sequence} || !artState.player.inCombat)`, 15000);
        await page.shot("cycle-" + cycle);
      }
      await page.waitFor(`artState.threats.find(t=>t.id===${JSON.stringify(id)}).health===0`);
      check(await page.evaluate("artPoses.length>1"), `${name} skeleton must animate`);
      check(await page.evaluate("artState.carriedSalvage===0"), "Rewards require looting the corpse");
      check(await page.evaluate(`artHistory.some(s=>s.threats.find(t=>t.id===${JSON.stringify(id)}).actionSequence>0)`), `${name} must execute its real combat action`);
      check(await page.evaluate("artState.player.health>0"), "Player survives encounter");
      // Walk to the corpse before opening the normal loot panel.
      if (await page.evaluate<number>(distance) > 2) {
        await approach(2);
      }
      await page.press("KeyF");
      await page.waitFor(`artState.lootOpenId===${JSON.stringify(id)}`);
      await page.waitFor('document.getElementById("loot-item").checkVisibility()');
      await page.click("#loot-item");
      await page.waitFor("artState.carriedSalvage>0");
      await page.shot("defeated-and-looted");
      check(page.errors.length === 0, "No browser exceptions");
      await Bun.write(`${page.output}/result.json`, JSON.stringify({ id, name, model, errors: page.errors, poses: await page.evaluate("artPoses.length"), health: await page.evaluate("artState.player.health") }, null, 2));

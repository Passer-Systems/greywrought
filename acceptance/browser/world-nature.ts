...
const locations=[{id:'lake',x:-4,z:-116},{id:'town',x:-8,z:-3},{id:'ridge',x:60,z:15},{id:'woods',x:-6,z:45}];
...
try{
 ...
 await enter();console.log('entered lake');await page.shot('lake-approach');
 await orbit(0,-85);await Bun.sleep(500);await page.shot('lake-shore-low');
 const performanceResults=[];
 for(const reflection of [true,false]){
  await page.evaluate(`(()=>{const lake=window.natureScene.getObjectByName('meadow-lake');window.reflectWater??=lake.onBeforeRender;lake.onBeforeRender=${reflection?'window.reflectWater':'()=>{}'};window.frameSamples=[];window.lastFrame=0;window.measuring=true;})()`);
  await Bun.sleep(4000);
  const samples=await page.evaluate<{duration:number;interval:number}[]>('window.measuring=false;window.frameSamples');
  const summary=(key:'duration'|'interval')=>{const values=samples.map(s=>s[key]).filter(Boolean).sort((a,b)=>a-b);return{median:values[Math.floor(values.length*.5)],p95:values[Math.floor(values.length*.95)]};};
  check(samples.length>20,'Lake performance produces frame samples');performanceResults.push({reflection,frames:samples.length,callbackMs:summary('duration'),frameMs:summary('interval')});
 }
 await page.evaluate("window.natureScene.getObjectByName('meadow-lake').onBeforeRender=window.reflectWater");
 await Bun.write(`${page.output}/lake-performance.json`,JSON.stringify(performanceResults,null,2));console.log('Lake performance',JSON.stringify(performanceResults));
 await orbit(0,85);
 await page.key('KeyW',true);await page.waitFor('Number(document.body.dataset.gamePlayerZ)>-100',12000);await page.key('KeyW',false);
 ...
 await page.waitFor('Number(document.body.dataset.gamePlayerZ)>-83',12000);await page.key('KeyW',false);await page.waitFor('document.body.dataset.rigAnimation==="Idle"');await page.shot('lake-exit');
 ...
 console.log('swimming complete');await switchTo('town');await page.shot('town-signs');

...
const character = { id: 'floating-fixture', name: 'Cast Tester', archetype: 'alchemist' as const, createdAtMillis: Date.now() };
...
try {
  ...
  check(damage.text==='10','Outgoing number must include the actual level bonus');
  ...
  check(await page.evaluate<string>('document.querySelector(\'.floating-combat-hit[data-kind="heal"]\').textContent')==='+6','Alchemist Defend must float its actual restored health');

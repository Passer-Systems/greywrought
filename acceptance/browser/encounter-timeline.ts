...
try {
 ...
 const queue = () => page.evaluate<{id:number;action:string;offsetSeconds:number}[]>('JSON.parse(document.getElementById("combat-plan").dataset.queued)');
 const full = await queue();
 const firstSelector = '.combat-plan-move[data-queue-id="'+full[0]!.id+'"]';
 const pointRemove = await page.evaluate<{x:number;y:number}>(`(()=>{const r=document.querySelector(${JSON.stringify(firstSelector)}).getBoundingClientRect();return {x:r.x+r.width/2,y:r.y+r.height/2}})()`);
 await page.call('Input.dispatchMouseEvent',{type:'mousePressed',...pointRemove,button:'right',buttons:2,clickCount:1});
 await page.call('Input.dispatchMouseEvent',{type:'mouseReleased',...pointRemove,button:'right',buttons:0,clickCount:1});
 check((await queue()).length===2,'Right-click must clear exactly the clicked slot');
 await page.press('KeyQ');
 check((await queue()).length===3,'A normal ability must refill the empty slot');
 const replace = (await queue())[1]!;
 const selected = '.combat-plan-move[data-queue-id="'+replace.id+'"]';
 await page.click(selected); await page.press('KeyE');
 check((await queue()).some(m=>m.id===replace.id&&m.action==='brace'&&m.offsetSeconds===replace.offsetSeconds),'Click then hotkey must replace in place');
 await page.click(selected); await page.click('[data-action="guard"]');
 check((await queue()).some(m=>m.id===replace.id&&m.action==='guard'&&m.offsetSeconds===replace.offsetSeconds),'Click then hotbar ability must replace in place');
 await page.click('.combat-plan-clear');
 await page.press('KeyQ');await page.press('KeyQ');await page.press('KeyQ');
 check((await queue()).length===3 && (await queue()).every(m=>m.action==='strike'),'Repeated ordinary keys must fill all three slots');
 await page.click('.combat-plan-move[data-offset="0"]');await page.press('KeyX');
 check((await queue()).every(m=>m.action==='strike'),'Invalid recovery replacement must preserve the original plan');
 await page.shot('queue-edits');

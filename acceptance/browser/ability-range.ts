...
try {
  ...
  page = await openBrowser('ability-range', { beforeNavigate: async call => {
    await call('Page.addScriptToEvaluateOnNewDocument', { source: `window.EventSource=class{};
      ...
      const Native=WebSocket; window.WebSocket=class extends Native {
        constructor(url,...args){super(String(url).includes('/world')?'ws://127.0.0.1:4196/world':url,...args);this.addEventListener('message',event=>{const d=window.decodeWorldMessage(event);if(d.type==='state')window.rangeState=d;});}
      };` });
  } });
  ...
  check(!await page.evaluate<boolean>(glowing('strike')), 'Distant Attack must not glow');
  ...
  check(await page.evaluate<boolean>(glowing('strike')), 'Attack must share 5 metre reach');
  ...
  await page.key('KeyS', true); await page.waitFor(`!${glowing('strike')}`); await page.key('KeyS', false);

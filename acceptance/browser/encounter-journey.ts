...
try {
  ...
  moves = await plan();
  check(moves.map(move=>move.offsetSeconds).join(',') === '0,1,2,3', "Paid moves and free fillers must each occupy one beat");
  ...
  await page.shot("five-beat-plan");
  await page.reload();
  ...
  await page.waitFor('document.querySelectorAll(".combat-plan-move").length === 5 && Number(document.body.dataset.gameReservedStamina) === 5');
  check(Number((await page.read()).gameReservedStamina) === 5, "Five one-stamina moves must fit the energy budget");
  await page.press('KeyV');
  check((await plan()).length === 5, "A free filler must not create a sixth move");
  await page.shot("full-five-move-plan");
